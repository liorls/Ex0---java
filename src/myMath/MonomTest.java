...
import static myMath.Monom.EPSILON;
...
public class MonomTest {
	...
	/* checks if zero and if String represenation will result equal monom */
	private static void test2() {
		...
		Monom_Comperator myComp = new Monom_Comperator();

		for(int i=0;i<monoms.size();i++) {
			Monom m = new Monom(monoms.get(i));
			String s = m.toString();
			Monom m1 = new Monom(s);
			int result = myComp.compare(m, m1);
			boolean e =  (result < EPSILON && result > -1 * EPSILON);
